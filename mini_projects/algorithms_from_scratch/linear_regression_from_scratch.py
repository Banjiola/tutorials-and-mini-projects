...
class OLS:
    """Implementaion of Linear Regression using Ordinary Least Squares"""
    def __init__(
            self, 
            X_train, 
            y_train
            ) -> None:
        self._X_original = X_train
        self.X_train = X_train
        self.y_train = y_train
        self.estimated_parameters = None

    def prepare(self):
        bias = np.ones(self._X_original.shape[0]) # Initialise bias term
        self.X_train = np.column_stack((bias, self._X_original)) # this is important to keep model from changing after every run
        return self.X_train
...
# Plot to show the convergence of a convex function
    ...
    print('='*30)
    print(X.info())
    df = df.dropna() # drop null entries
    X = df.drop(columns='mpg') # select X and y
    y = df['mpg']
